...
static public class MyParticleSystem{
  ...
  public float spwan_scale = 3.0f;
  public float point_size  = 0.9f;
  
  public Param param = new Param();
  
  static public class Param{
    public float dissipation = 1.20f;
    public float inertia     = 0.10f;
  }
  ...
  public void resize(DwPixelFlow context, int num_particels_x, int num_particels_y){
    ...
    // allocate texture
    tex_particles.resize(context, GL2ES2.GL_RGBA32F, particles_x, particles_y, GL2ES2.GL_RGBA, GL2ES2.GL_FLOAT, GL2ES2.GL_NEAREST, 5, 5);
  ...
  public void reset(){
    ...
    spawn(null, -1, -1, -1, particles_x *particles_y);
  ...
  public void render(PGraphics2D dst, int background){
    ...
    dst.beginDraw();
    dst.blendMode(PConstants.MULTIPLY);
    if(background == 0) dst.blendMode(PConstants.SCREEN); // works nicely on black background
    ...
    shader_particelRender.drawFullScreenPoints(num_points_to_render);
